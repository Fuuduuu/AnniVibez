...
- `RUNTIME_CUTOVER_C7 = FINAL PREVIEW GATE`: C7 remains external fixed-preview deployment plus a real Android installed-PWA human gate only, with no repository source, test, package, configuration, workflow or deployment-config changes. Fixed branch/origin: `c7-storage-cutover` / `https://c7-storage-cutover.annivibe.pages.dev`; accepted legacy recovery preview deployment `342c1212-7023-48e5-a195-be0f3896748c`, provenance `6123c12567efea7040d5a5995dd8ba5f738c73a1`; Human Step 1 is PASS and `PLACE_ORDER_BEFORE` remains Kodu — Õie 58, Vanaema — Kaevu 10, Trenn — Pikk 23. Manual C7 forensic/destructive drills are `WAIVED BY HUMAN FOR FINAL ACCEPTANCE GATE`, not executed: raw localStorage SHA, mandatory remote DevTools storage inspection, quota, IndexedDB/legacy-key deletion, deliberate divergence and rollback/re-forward phone drills. The required final gate is exact forward `9dac8c021be1b99bf7ec1227615cb6f3bd874674`, complete accepted C6 automation, focused source review, stable/remote-verified PWA artifact, same-alias preview deployment and final Android continuity/CRUD/persistence smoke. C8: LOCKED.

The C6 implementation is accepted. Cloudflare Pages nevertheless auto-deployed C6-containing `main` commits before C7/C8 governance: canonical production is now the frozen accidental deployment `0cca08f2-3a87-4176-99e2-7bc107bf8ce5` from `549203d3a0eced020fc954cab62721ee85a06936`, not a C8 accepted deployment. Automatic production deployments are independently verified disabled (`production_branch: main`, `production_deployments_enabled: false`, preview setting `all`, Git integration `github`); a `main` push MUST NOT auto-deploy production, and any unexpected production deployment before C8 is a STOP. No pre-C6 rollback is performed because clients may already have switched authority to IndexedDB. C7 may resume only on its later fixed preview origin; C8 is the separate intentional production-deployment gate. D1/backend, authentication, sync and outbox UI remain locked.
