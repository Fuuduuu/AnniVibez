...
- `RUNTIME_CUTOVER_C7 = STEP 1 CHECKPOINTED`: C7 remains external fixed-preview deployment plus a real Android installed-PWA human gate only, with no repository source, test, package, configuration, workflow or deployment-config changes. Fixed branch/origin: `c7-storage-cutover` / `https://c7-storage-cutover.annivibe.pages.dev`; accepted legacy recovery preview deployment `342c1212-7023-48e5-a195-be0f3896748c`, provenance `6123c12567efea7040d5a5995dd8ba5f738c73a1`; the earlier incomplete preview was `3fdf1f45-9c64-4d70-b5d9-05a0fb9225b2`. Human Step 1 is PASS: installed PWA operation, one-off and recurring events, household profile, three visible saved places, reminders and manual waste schedule; `WASTE_IMPORT: NOT AVAILABLE`; no unexpected state. Saved-place add/remove is `NOT USER-EXPOSED`; the UI exposes edit/save only. `PLACE_ORDER_BEFORE` is locked as Kodu — Õie 58, Vanaema — Kaevu 10, Trenn — Pikk 23. C7 Step 2 and Steps 3-7 remain PENDING; next safe action is Step 2A pre-cutover legacy-data existence and baseline verification before any forward deployment. C8: LOCKED.
