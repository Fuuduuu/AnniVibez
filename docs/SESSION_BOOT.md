...
- Current implementation authority is only `functions/_lib/auth.js` and `scripts/backend/auth.test.mjs`. Phase 5 parses and validates the `Authorization: Bearer` credential, hashes it with `hashSecret("device-session", token)`, calls `findActiveSessionByHash(...)`, makes the authentication decision, maps credential failure separately from infrastructure failure, and constructs the trusted authentication context. Phase 6+ remains locked.
