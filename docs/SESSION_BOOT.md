...
- `RUNTIME_CUTOVER_C6_SCOPE_OPEN = OPEN` (docs-only pass at baseline `f919c7f6bef6fbfed15e20609c8cd2e12cdbb878`; C6 NOT implemented, NOT checkpointed). C6 wires the accepted storage foundation into the runtime: `LEGACY` -> existing localStorage repositories, `READY` -> C5 IndexedDB repositories. Exact scope: 13 production files (`src/storage/storageAuthority.js` limited to one additive read-only `getReadyAuthorityIdentity()` accessor, added by `RUNTIME_CUTOVER_C6_AUTHORITY_IDENTITY_SCOPE_AMEND` at baseline `8e15c47e3d29b3cbcde88c7d42e55ceba720715c` so C5 repositories bind to the exact switch identity C4 mounted `READY` against, never a C6-side meta re-read; plus `src/main.jsx`, `src/App.jsx`, `src/calendar/useHouseholdEvents.js`, `src/waste/useHousehold.js`, `src/hooks/useSavedPlaces.js`, `src/hooks/useSettings.js`, `src/components/SeadedTab.jsx`, `src/components/EventDialog.jsx`, `src/components/HouseholdSettings.jsx`, `src/components/WasteSettings.jsx`, new `src/components/StorageStatus.jsx`, `src/design/shell.css`) and 6 test files (`scripts/shell/app-shell.test.mjs`, `scripts/calendar/browser-cases.mjs`, `scripts/waste/browser-cases.mjs`, `scripts/reminders/browser-cases.mjs`, `scripts/storage/storage.test.mjs`, `scripts/storage/indexeddb-browser.test.mjs`), the sixth added by `RUNTIME_CUTOVER_C6_REMINDER_TEST_SCOPE_AMEND` (docs-only, baseline `07a74aafc174aea3aab43aa99d60e8721f396553`): the reminder cases read calendar state from the frozen legacy key, so they need a mode-aware test-only `readCalendarEvents` helper; the reminder wrappers stay run-only and a legacy-key mirror workaround is forbidden. Full locked contract in `docs/ACTIVE_SCOPE_LOCK.md` (Runtime cutover C6) and `docs/ACCEPTED_CHECKPOINTS.md` (`RUNTIME_CUTOVER_C6_SCOPE_OPEN`). Runtime behavior change: YES once implemented and accepted; C6 checkpoint requires automated GREEN, independent source review PASS / ACCEPT and a desktop Chrome human smoke PASS. C7-C8: LOCKED.
- Current gate: `RUNTIME_CUTOVER_C6` (C6 SOURCE SCOPE: OPEN). C7-C8: LOCKED.

Runtime cutover implementation is complete through C5 (five dormant storage modules; no runtime behavior change). C6 source scope is open, but C6 is not implemented: until an implemented, reviewed, smoke-passed and checkpointed C6 exists, the accepted runtime baseline is unchanged and the running Majandus application continues using its existing accepted localStorage/runtime paths. C7-C8 remain locked. D1/backend, authentication, sync and outbox UI remain locked.
