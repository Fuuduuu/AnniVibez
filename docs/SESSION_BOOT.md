...
- Phase 8 is accepted at `d65195356bd2430d7e634c3959b7a5dbe79a12bd`; its independent audit was PASS with zero blocking, material, or minor findings, and regression evidence was 12 focused / 74 combined passes with zero failures or skips. The accepted files `functions/api/auth/session.js` and `scripts/backend/session-api.test.mjs` are protected. Evidence covers direct handlers only, not actual Pages dispatch.
- Phase 9 uses the already-installed Wrangler `4.135.0` Pages dev local runtime and the exact Task 9 test-only allowlist in `docs/ACTIVE_SCOPE_LOCK.md`; no package or runtime-configuration change is authorized or required. Actual Pages routing for both auth routes remains unverified, including session HEAD behavior; the Phase 9 tests must discover and pin runtime behavior.
- Phase 10+ implementation remains locked. Remote D1, Pages binding/migration, deployment, credentials, GitHub workflows, Wrangler deployment automation, client/runtime integration, and real-user readiness remain locked.
