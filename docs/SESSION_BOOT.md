...
- `RUNTIME_CUTOVER_PLAN = ACCEPTED` (human) in `docs/superpowers/plans/2026-09-16-majandus-runtime-cutover.md` at plan checkpoint `8077c0e2626f1be1ef149a5690f43aa1e46cb248`; `RUNTIME_CUTOVER_PLAN_FRESH_REVIEW = PASS`. Four earlier independent fresh reviews returned AMEND.
  ...
  - Implementation phases: C1-C4 CHECKPOINTED; C5-C8 LOCKED.
...
- Current gate: `RUNTIME_CUTOVER_C5`. C5 source scope: OPEN at `590a2a7c56f63a5895a4fd56da7668ad879b95c6`.
  - Production files: new `src/storage/replicaRepositories.js`; `src/storage/localReplica.js` restricted to one additive read-only `listCalendarEvents()` accessor (see the amendment below).
  ...
  - `RUNTIME_CUTOVER_C5_CALENDAR_READ_SCOPE_AMEND` at baseline `02b5ff1824258bf28d16121517c072d90d0b892a`: discovered during C5 TDD, before the calendar RED→GREEN cycle, that the accepted `localReplica.js` read surface (`getHouseholdProfile`, `getWasteState`, `getMeta`, `listSharedPlaces`) has no collection-read accessor for `calendarEvents`, and `replicaRepositories.js` must never call `replica.transact`/`runTransaction` directly (it is not on the direct-transaction-caller allowlist, which stays exactly `localReplica.js`, `legacyMigration.js`, `runtimeWrites.js`, `storageAuthority.js`). Resolution: one additive `listCalendarEvents()` accessor in `localReplica.js`, mirroring the accepted `listSharedPlaces()` pattern (canonical `id` order, zero writes, no calendar semantic logic added). This does not weaken the C3 mutation/transaction boundary. Full contract in `docs/ACTIVE_SCOPE_LOCK.md`.
  - Implementation at final HEAD `5c3f6fee68a5af98390f28706dc759d4737f5a56`; the independent C5 source review of that HEAD returned **AMEND** (`RUNTIME_CUTOVER_C5_REVIEW_AMEND`). Five findings, all requiring a corrective source pass before checkpoint: (A) an invalid *existing* places collection (e.g. two records sharing `order: 0`) could be silently auto-repaired into a valid contiguous order by a normal mutation, because the current-collection order was never re-validated before planning; (B) the parity tests asserted hand-computed expected values instead of comparing against a true legacy-repository oracle run on equivalent seeded state; (C) the failure matrix covered `QuotaExceededError` but not a separate generic/other write error; (D) the dependency allowlist omitted `requestResult` from `./indexedDb.js`, which C5's `runReplicaMutation(...).read(...)` callbacks genuinely need and which is now explicitly allowed, scoped only to that use; (E) the calendar canonical-order comparator used locale-sensitive `localeCompare()` instead of the same locale-independent `<`/`>` comparator C4's revert/export already uses. Corrective source pass locked to exactly `replicaRepositories.js` (finding A) and `localReplica.js` (finding E); no other production behavior change. Full contract in `docs/ACTIVE_SCOPE_LOCK.md`.
  - C5 status: OPEN / NOT CHECKPOINTED, AMEND required. C6-C8: LOCKED. Full locked architecture, contracts and cumulative test scope in `docs/ACTIVE_SCOPE_LOCK.md`.

The only open implementation scope is runtime cutover C5 (one new dormant storage module implementing the IndexedDB domain repositories, plus one additive read-only accessor in `localReplica.js` per the amendment above; no runtime behavior change); implementation happens in a separate pass, and C6-C8 remain locked. Runtime implementation, startup migration, D1/backend, authentication, sync and outbox UI remain locked. The running Majandus application continues using its existing accepted localStorage/runtime paths.
