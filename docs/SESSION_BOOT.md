...
- Current gate: `RUNTIME_CUTOVER_C6_SCOPE_OPEN`, pending its own separate docs-only scope-open pass. C6-C8: LOCKED.

Runtime cutover implementation is complete through C5 (five dormant storage modules; no runtime behavior change). C6 opens only through its own separate docs-only scope-open pass, and C7-C8 remain locked. Runtime implementation, startup migration, D1/backend, authentication, sync and outbox UI remain locked. The running Majandus application continues using its existing accepted localStorage/runtime paths.
