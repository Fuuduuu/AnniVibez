...
**MAJANDUS_BACKEND_AUTH_FOUNDATION_P7_CREATE_HOUSEHOLD_ENDPOINT**: **OPEN FOR IMPLEMENTATION** at `63981e14903cac077471b6e6911664d73cd94448`.
...
- Current implementation authority is only `functions/api/auth/create-household.js` and `scripts/backend/create-household-api.test.mjs`. The Pages route is `/api/auth/create-household`: `onRequestPost({ request, env })` performs the POST path, while generic `onRequest` returns 405 with `Allow: POST` for every other method, including HEAD and OPTIONS. It accepts only the existing capped JSON transport and exact create-household body, requires `env.DB`, calls the accepted Phase 6 service once, projects with `publicHouseholdCreation(...)`, and returns 201 only for the initial token/recovery response. Invalid request/input maps to `INVALID_REQUEST`/400; missing binding or unexpected failure maps to generic `INTERNAL_ERROR`/500; the existing no-store JSON error envelope carries no sensitive detail. There is no Phase 7 conflict/409, duplicate-name/address, retry, replay, mutation-key, SQL, hash, credential-generation, or persistence logic.
- Phase 8+ remains locked, including the session endpoint, integration regression, external execution, invite/join, recovery, device linking/revocation, sync, outbox, client auth/runtime integration, account management, and legacy cleanup. Remote D1, Pages binding/migration, deployment, credentials, GitHub workflows, and Wrangler deployment automation remain locked; no real-user readiness is granted.
