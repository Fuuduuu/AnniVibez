...
- Current implementation authority is only `functions/_lib/households.js` and `scripts/backend/households.test.mjs`, exporting exactly `createHousehold({ db, input, clock })` and `publicHouseholdCreation(result)`. Phase 6 generates IDs, the plaintext device token and the recovery code, derives their hashes, takes one timestamp, sets household revision `1`, constructs the persisted creation record, runs one atomic repository creation operation, and returns the public creation result. The repository receives only `tokenHash` and `recoveryHash`; `change_log` and `applied_mutations` are not written. Creation is NON-IDEMPOTENT and an unknown-result create must not be auto-retried. Phase 7+ remains locked.
