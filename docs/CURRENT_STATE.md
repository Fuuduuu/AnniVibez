...
- `VISUAL_POLISH_V2` implementation (V2 source scope OPEN with exact files in `docs/ACTIVE_SCOPE_LOCK.md`; Visual Polish V1 CHECKPOINTED; runtime cutover plan HUMAN ACCEPTED; C1-C8 LOCKED; Phase A IndexedDB foundation ACCEPTED).
...
- The runtime cutover plan in `docs/superpowers/plans/2026-09-16-majandus-runtime-cutover.md` (all six acceptance items resolved) is HUMAN ACCEPTED at plan checkpoint `8077c0e2626f1be1ef149a5690f43aa1e46cb248`, after four AMEND verdicts, four amendments (plan Section 12) and a final fresh independent review PASS. Its phases C1-C8 remain LOCKED; each opens only through a separate scope-open pass. Visual Polish V1 is ACCEPTED / CHECKPOINTED (implementation `f367f2c06f3a4d5e96abb8ca7f4f8d96028491ee`, review PASS, behavior and storage/runtime changes NONE). The current gate is Visual Polish V2, the final visual-only interlude before C1; its source scope is OPEN at baseline `142e7fb105d09c46ad810b00c9475b32a7088436` with the exact files in `docs/ACTIVE_SCOPE_LOCK.md` (`src/design/shell.css`, `src/design/calendar.css`, `src/design/waste.css`, `src/components/SeadedTab.jsx`, `src/components/EventDialog.jsx`; authorized behavior and storage/runtime changes NONE), and implementation happens in a separate pass (direction: B-lite; modern native Android; calm warm utility; Material-3 influenced; no dark mode; no storage/runtime changes). No runtime/storage implementation authority is open until the V2 checkpoint and an approved C1 scope-open pass. Legacy localStorage remains the runtime authority. Runtime cutover implementation, startup migration, dual-write, D1/backend, authentication, sync, outbox UI, dependency and deployment work remain locked.
