...
- Phase 7's accepted files are `functions/api/auth/create-household.js` and `scripts/backend/create-household-api.test.mjs`; its direct-handler contract is accepted, but actual Pages dispatch is NOT yet accepted and remains mandatory Phase 9 work.
- Phase 8 is ACCEPTED at `d65195356bd2430d7e634c3959b7a5dbe79a12bd` (`feat: add Majandus session endpoint`): independent final audit PASS; BLOCKING `0`; MATERIAL `0`; MINOR `0`; focused tests `12 passed`, combined Phase 1–8 regression `74 passed`, both with `0 failed` and `0 skipped`. Its accepted files are protected, and its test evidence covers direct exported-handler contracts only; actual Pages dispatch was not verified.
- `MAJANDUS_BACKEND_AUTH_FOUNDATION_P9_ROUTING_VERIFICATION` is OPEN FOR IMPLEMENTATION only for the Task 9 test files: create `scripts/backend/foundation-integration.test.mjs`; modify `scripts/backend/migration-contract.test.mjs`, `scripts/backend/crypto.test.mjs`, `scripts/backend/auth.test.mjs`, and `scripts/backend/households.test.mjs`. The approved local mechanism is the existing Wrangler `4.135.0` Pages dev runtime; actual Pages file/method dispatch for both auth routes remains unverified work, including actual session-route HEAD behavior.
- Phase 10+ remains LOCKED. No package/runtime-config changes, remote D1, external deployment, Cloudflare credentials, client/runtime coupling, or real-user readiness is authorized.
- Next safe action: implement the exact Phase 9 test-only scope using local Wrangler Pages dev; stop if runtime behavior conflicts with the current handler contract.
...
- The runtime cutover plan remains HUMAN ACCEPTED and C1-C8 are ACCEPTED / CHECKPOINTED. C8 is the accepted production runtime; the earlier accidental C6 deployment remains historical incident evidence and was not rolled back to pre-C6. Phase 8's exact two-file local session-handler scope is open; Phase 9+ routing/integration, sync, outbox UI, legacy cleanup/deletion, and automatic-production-deployment policy remain separate locked scopes.
