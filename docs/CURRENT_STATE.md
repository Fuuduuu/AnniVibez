...
- Backend Phases 1–4 are ACCEPTED: Phase 1 at `6d3496177a6c8ca1857c4081d900be69a0bbfeeb`; Phase 2 at `8a696c240bfbfb93395b85a98b55fbfeb487e328`; Phase 3 at `9a1bb5aea3a15a844a38d8651904740e953e20e7`; the Phase 4 repository-contract checkpoint at `915ed9a320e4b5395f6e007d6af2db92c5eeb95d`; and Phase 4 implementation at `50faf24667e560295dab37a26a33272b1ffaee1a` (`feat: add Majandus D1 repository foundation`). Phase 4 final audit: PASS; findings: none; Phase 1–4 regression: `37 passed`, `0 failed`, `0 skipped`.
- `MAJANDUS_BACKEND_AUTH_FOUNDATION_P5_DEVICE_AUTHENTICATION` is OPEN FOR IMPLEMENTATION only for `functions/_lib/auth.js` and `scripts/backend/auth.test.mjs`. Its accepted boundary is bearer parsing and syntax handling, `hashSecret("device-session", token)`, `findActiveSessionByHash(...)`, authentication decision, separate 401 versus infrastructure-failure mapping, and trusted authentication-context construction. Phase 4 remains precomputed-hash repository work only; it does not parse bearer credentials, hash plaintext tokens, or decide authentication. Phase 6+ remains locked.
- Next safe action: `MAJANDUS_BACKEND_AUTH_P5_DEVICE_AUTHENTICATION`. No live backend integration, remote D1, deployment, client/runtime coupling, or Phase 6+ work is authorized.
...
- The runtime cutover plan remains HUMAN ACCEPTED and C1-C8 are ACCEPTED / CHECKPOINTED. C8 is the accepted production runtime; the earlier accidental C6 deployment remains historical incident evidence and was not rolled back to pre-C6. Only Phase 5's exact local authentication scope is open; Phase 6+, sync, outbox UI, legacy cleanup/deletion, and automatic-production-deployment policy remain separate future scopes.
