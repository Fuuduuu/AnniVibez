...
**RUNTIME_CUTOVER_C5_REVIEW_AMEND** (baseline `5c3f6fee68a5af98390f28706dc759d4737f5a56`): the independent C5 source review of the implementation at that HEAD returned **AMEND**. Five findings, all requiring a corrective source pass before C5 can be checkpointed; C5 stays OPEN / NOT CHECKPOINTED in the meantime.
- **A — invalid places collection can be auto-repaired.** `planPlacesMutation` validated every individual `sharedPlaces` record but never validated the *existing collection* with `validateSharedPlaceOrders(records)` before sorting and planning. An already-invalid collection (for example two records both at `order: 0`) could therefore be silently reindexed into a valid contiguous order by a normal mutation, violating the accepted "invalid stored data is not auto-repaired" contract. Corrective contract: before any places mutation transforms the current state, validate each record and `validateSharedPlaceOrders(current records)`; if the current collection is invalid, the mutation rejects with zero `sharedPlaces` writes and zero authority/`commitCount` change, and the stored invalid bytes are left exactly as they were (the existing `load()` `writable: false`/no-repair behavior is unchanged). Required test: a real `repos.places.update/add/remove` call against a seeded duplicate/invalid-order collection must reject, with a before/after deep-comparison of domain and authority state — a direct `runReplicaMutation` synthetic-bad-plan test does not substitute for this regression (the existing C3 resulting-order guard test stays, in addition).
- **B — the parity matrix must be a true oracle comparison, not asserted expected values.** For every required case (household `save`; places `update`/`add`/`remove`/padding; calendar+waste `create`/update occurrence/update series/`remove`/`importWaste`), the test must seed the legacy repository and the IndexedDB repository from semantically equivalent state, run the *same* operation on both through their own accepted code paths, and compare the resulting semantic state — not compare against a value the test author computed and typed in by hand. Household cases include the accepted normalization/default cases, and must verify any retained IndexedDB extras stay semantically unchanged. Places cases use the C2 neutral module as the canonical oracle and assert separately: existing ids preserved, new ids only for newly materialized entries, and contiguous `0..n-1` orders (ids are an IndexedDB concern, not part of legacy semantic equality, except for those explicit identity assertions). Calendar+waste cases build two independent deterministic worlds — (A) `createEventRepository(memoryStorage, deterministicLegacyNewId)` and (B) the C5 repository with a deterministic id sequence producing equivalent ids — seeded with semantically equivalent events/`wasteImports`/extras, run the same operation on each, and compare events as id-keyed maps/sets (never by incidental array order), plus `wasteImports`, extras, `version` and writable/error semantics; coverage must include recurrence, `excludedDates`, `overrides`, imported-event restrictions, waste import behavior and extras preservation.
- **C — a generic write-error case is required in addition to `QuotaExceededError`.** The accepted failure matrix requires both; add a second write-request-level failure mode (for example a `DOMException` named `UnknownError`) exercised through a real C5 repository mutation, with the same invariant as the quota case: the mutation rejects, domain state and `authority.commitCount` are byte-identical before/after, zero partial writes, zero legacy fallback. The existing quota cases stay.
- **D — `requestResult` is an explicit, accepted C5 dependency.** The C3 transaction contract already requires `READ` to await request promises through `requestResult`, and C5's `runReplicaMutation(...).read(...)` callbacks genuinely need it for exactly that purpose. The dependency allowlist below is amended to name `requestResult` from `./indexedDb.js` explicitly, scoped only to request promises issued from the stores passed into `read(...)`. This amendment grants nothing else: `replicaRepositories.js` still may not call `replica.transact(...)`, `runTransaction(...)`, `openMajandusDb(...)`, touch schema, or open a database directly, and the direct-transaction-caller allowlist is unchanged.
- **E — the canonical event-id order must be locale-independent.** `listCalendarEvents()`'s normative shape used `left.id.localeCompare(right.id)`, which is locale-sensitive and not guaranteed identical to the locale-independent `<`/`>` comparator C4's revert/export already uses (`left.id < right.id ? -1 : left.id > right.id ? 1 : 0`) for arbitrary valid event ids (the accepted event model does not restrict ids to UUID characters). Corrective contract: `listCalendarEvents()` and the C5 calendar envelope reconstruction both use the same `<`/`>` comparator as C4; C4's revert/export code itself is unchanged. Required test: use event ids where locale collation could plausibly differ from relational string ordering, deriving the expected order from the `<`/`>` comparator, not `localeCompare()`; required falsification check: temporarily replace the comparator with `localeCompare()` and confirm the canonical-order regression test fails, then restore — removing the explicit sort is not an acceptable substitute mutation, since IndexedDB's own primary-key enumeration may already happen to return canonical order regardless.

The corrective source pass is locked to exactly: `replicaRepositories.js` (reject a mutation from an invalid existing places collection) and `localReplica.js` (replace the `localeCompare` canonical sorter with the locale-independent comparator) — no other production behavior change. All existing C5 tests are preserved unless a redundant assertion is cleanly folded into a stronger oracle test.
...
**Quota / failure contract:** an injected `QuotaExceededError` *and* a separate generic/other write error (finding C) both atomically preserve the previous IndexedDB state — no partial domain mutation, no accidental `commitCount` increment, the previous snapshot preserved, and the mutation promise rejects. No fallback to legacy writes.

**C5 parity test matrix (Section 8 C5; finding B — a true oracle comparison, not asserted expected values):** for every operation, seed the legacy repository and the IndexedDB repository from semantically equivalent state, run the *same* operation on each through its own accepted code path, and compare the resulting semantic state produced by each; calendar events are compared as id-keyed maps/sets, never by array-position equality. Required parity cases:
- household: `save`, including the normalized/default cases already supported by the accepted legacy repository, with retained IndexedDB extras verified semantically unchanged;
- places: update, add, remove, padding/default persistence, using the C2 neutral module as the canonical oracle; assert separately: stable existing ids, new ids only for newly materialized entries, and contiguous `0..n-1` orders (ids are an IndexedDB concern, not part of legacy semantic equality except for these explicit identity assertions);
- calendar + waste: create, update occurrence, update series, remove, `importWaste`, using two independent deterministic worlds — (A) `createEventRepository(memoryStorage, deterministicLegacyNewId)` and (B) the C5 repository with a deterministic id sequence producing equivalent ids — seeded with semantically equivalent events/`wasteImports`/extras; compare events as id-keyed maps/sets, `wasteImports`, extras, `version` and writable/error semantics; coverage includes recurrence, `excludedDates`/`overrides`, imported-event protections, `wasteImports` semantics and extras preservation. No new semantics are created to satisfy IndexedDB.

**Required C5 failure/concurrency tests (minimum):** quota failure leaves previous state; a separate generic/other write error also leaves previous state (finding C); authority mismatch writes nothing; malformed authority writes nothing; invalid planned record writes nothing; an invalid resulting places order writes nothing (the inherited C3 resulting-order guard); an invalid *existing* places collection rejects the mutation before any write, via a real repository call, not a synthetic bad-plan substitute (finding A); the C3 thenable/async plan protection is inherited, not reimplemented; two-tab concurrent household mutation; two-tab concurrent places mutation; two-tab concurrent calendar mutation. Real Chromium IndexedDB is used where transaction/concurrency behavior is material; Node/stub tests may cover pure conversion/parity logic.

**Canonical event-id order (finding E):** `listCalendarEvents()` and the C5 calendar envelope reconstruction use the same locale-independent comparator C4's revert/export already uses — `left.id < right.id ? -1 : left.id > right.id ? 1 : 0` — never `localeCompare()`, which is locale-sensitive and not guaranteed identical for arbitrary valid event ids.
...
- the C5 corrective source pass required by `RUNTIME_CUTOVER_C5_REVIEW_AMEND` (findings A-E above), limited to exactly `replicaRepositories.js` (reject a mutation from an invalid existing places collection) and `localReplica.js` (locale-independent canonical event-id comparator), plus the two C5 test files
- a further C5 independent review of the corrective pass, and a docs-only checkpoint recording it once that review returns PASS/ACCEPT
...
NEXT: implement the C5 corrective source pass required by `RUNTIME_CUTOVER_C5_REVIEW_AMEND` (findings A-E), then seek a further independent C5 review. The runtime cutover plan is ACCEPTED and C1-C4 are CHECKPOINTED; C5 remains OPEN / NOT CHECKPOINTED pending that corrective pass and review; C6-C8 remain LOCKED.
