...
- Unchanged: the 12-file production scope, the C6 architecture (`LEGACY` -> existing localStorage repositories, `READY` -> accepted C5 repositories; C4 owns authority/state decisions, C6 owns browser signal wiring and runtime mounting), the importer allowlist and C7-C8 LOCKED. No `src/storage/**` file (including `storageAuthority.js`, `replicaRepositories.js`, `localReplica.js`) becomes writable.
- Implementation note (not an authorization): this amendment authorizes no workaround for obtaining `READY` authority identity. C5 repositories must still bind to the switch identity proven by the C4 boot. If that cannot be achieved through C4's already-public controller/replica API without changing a closed storage file, STOP; do not silently widen production scope.
...
- C6 implementation, strictly inside the exact 12 production files and 6 test files listed in the Runtime cutover C6 section
