...
- **C7 final preview / Android-PWA gate (mandatory; stays PENDING until the human confirms):**
  ...
  2. Before the final Android smoke, build exact forward provenance `9dac8c021be1b99bf7ec1227615cb6f3bd874674` with `VITE_STORAGE_AUTHORITY_MODE` absent; rerun the complete accepted C6 automated matrix; perform a fresh focused source review; prove complete/stable PWA artifacts; deploy only to the same fixed `c7-storage-cutover` alias; and prove remote/local PWA artifact equality with canonical production unchanged.
  ...
  4. In the existing installed Android PWA after the forward deployment, verify: existing one-off and recurring events, household data, saved-place values/order, reminder configuration and manual waste schedule remain; calendar create/edit/delete works; household and saved-place edit/save works; closing and reopening preserves migrated and new data; and no unexpected `STORAGE_LOST`, `LEGACY_DIVERGED`, `STORAGE_UNAVAILABLE`, `DOMAIN_INVALID`, `BLOCKED`, `AUTHORITY_HINT_PENDING`, `REVERT_FAILED`, `TransactionInactiveError` or equivalent visible storage failure appears.
