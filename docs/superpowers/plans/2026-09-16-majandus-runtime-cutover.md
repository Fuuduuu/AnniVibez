...
- C6 scope-open note (no change to the plan's design): the C6 file lists above are opened exactly as written in `docs/ACTIVE_SCOPE_LOCK.md`; the reminder, native-notification and saved-places suites are run-only (the reminder UI and native suites run through `scripts/shell/app-shell.test.mjs`); Section 9 item 5 (update in a second tab) is read with Section 4 — an ordinary committed update refreshes the other `READY` tab by domain re-read, and the reload banner is the required behavior for authority/connection changes.
