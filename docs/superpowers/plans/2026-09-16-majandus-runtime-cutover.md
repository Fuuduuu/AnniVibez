...
| C8 (SCOPE OPEN) | One later explicit manual controlled production deploy from exact C7-tested forward provenance; deployment NOT YET RUN | separate deploy pass; no source/config change | production |
...
- **C8 production** is SCOPE OPEN. It must not relabel accidental production; its later deploy pass is the only authorized route to a deliberate production boundary from `9dac8c021be1b99bf7ec1227615cb6f3bd874674` in forward/default mode with `VITE_STORAGE_AUTHORITY_MODE` absent. Deployment and acceptance remain PENDING in this scope-open pass.
