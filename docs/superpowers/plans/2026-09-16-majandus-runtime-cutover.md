...
| C7 (ACCEPTED / CHECKPOINTED) | Fixed-alias forward preview and Android/PWA human gate PASS; forensic/destructive rollback drill waived and not executed | existing external Pages infrastructure only; no repository source or deployment-configuration change | preview origin only |
| C8 (ACCEPTED / CHECKPOINTED) | Intentional manual controlled production deploy from exact C7-tested forward provenance; deployment `0c4e39e4-63b8-4e9a-8ac4-7a667bda6b80` passed | separate deploy pass; no source/config change | canonical production `https://annivibe.pages.dev` |
...
- C7 scope-open note (historical): before Step 1, one existing Cloudflare Pages preview/branch-alias origin was selected and fixed for the legacy seed and forward artifact. C7 final acceptance is recorded in Section 9; intentional canonical production deployment, C8 and all repository source/deployment configuration remained outside C7. After the C7 containment checkpoint, a `main` push MUST NOT auto-deploy production; any unexpected production deployment before an accepted C8 deployment scope is a STOP.
...
- **C7 final preview / Android-PWA gate (ACCEPTED / CHECKPOINTED):**
  1. Human Step 1 is recorded PASS on the fixed alias: legacy seed data includes one-off and recurring events, household profile, three visible saved places in the locked order, reminder configuration and a manual waste schedule; `WASTE_IMPORT: NOT AVAILABLE` is valid evidence.
  2. The final forward provenance `9dac8c021be1b99bf7ec1227615cb6f3bd874674` was built with `VITE_STORAGE_AUTHORITY_MODE` absent. The complete accepted C6 automated matrix, fresh focused source review (PASS; no blocking findings), build, complete/stable PWA artifact gate and remote/local byte proof all passed. Deployment `82f17c50-78de-4ef6-ba3a-185c9475f5ad` is `preview` on the same fixed `c7-storage-cutover` alias; canonical production remained unchanged.
  3. **MANUAL C7 FORENSIC/DESTRUCTIVE DRILLS: WAIVED BY HUMAN FOR FINAL ACCEPTANCE GATE.** The waived actions are raw localStorage SHA snapshots, mandatory remote DevTools storage inspection, quota simulation, manual IndexedDB deletion, manual shared-legacy-key deletion, a deliberate `LEGACY_DIVERGED` drill, and destructive rollback/re-forward phone drills. They are not claimed as executed; accepted automated/runtime coverage remains required.
  4. In the existing installed Android PWA after the forward deployment, the human recorded PASS: the forward preview opened; seeded legacy data, saved-place continuity, reminder configuration and manual waste schedule remained; Calendar create/edit/delete, household and saved-place edit/save, and close/reopen persistence passed; unexpected visible storage/error state was NONE.
- **C8 production (ACCEPTED / CHECKPOINTED):** the controlled manual Wrangler deployment `0c4e39e4-63b8-4e9a-8ac4-7a667bda6b80` deliberately established canonical production from exact forward/default provenance `9dac8c021be1b99bf7ec1227615cb6f3bd874674` with `VITE_STORAGE_AUTHORITY_MODE` absent. C8 production-critical validation, PWA artifact proof, remote `11/11` byte/hash proof and short production smoke passed. The accidental C6 deployment remains historical incident evidence; the C7 forensic/destructive waiver remains not executed.
