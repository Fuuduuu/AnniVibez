...
- Separate future recovery finding, RECORDED BUT NOT OPEN: a reset may remove the PIN, entry removal may fail, and PIN compensation may also fail; entries can then survive without a PIN and a later successful fresh PIN setup/save may overwrite those orphaned entries. This double-fault orphaned-diary risk was classified as a separate future finding, NOT an R1 violation and NOT automatically assigned to R2. It needs a separately bounded recovery/data-integrity decision before implementation. Other R1 MINOR backlog, also not R2 scope: PIN read denial can resemble no PIN; some PIN errors lack live/alert semantics; per-field draft coverage is partial; failed-save retry permanent coverage is absent; `useDiary.changePin` is unused; minor test naming/hygiene. Accessibility semantics may be addressed in later UI-V2 accessibility work.
