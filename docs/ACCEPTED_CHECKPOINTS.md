...
- C6 production scope: UNCHANGED (12 files; no `src/storage/**` file becomes writable); C6 architecture unchanged; the amendment authorizes no workaround for `READY` authority identity: C5 repositories bind to the switch identity proven by the C4 boot, and if that needs a closed storage file changed, STOP
- this pass was docs/governance only; no source, test, package or config file changed
